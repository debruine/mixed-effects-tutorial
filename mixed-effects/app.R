...
about_tab <- tabItem(
  tabName = "about_tab",
  h3("About this App"),
  p("Understanding mixed effects models through simulating data ")
)

## UI ----
ui <- dashboardPage(
  dashboardHeader(title = "Understanding mixed effects models through simulating data "),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Tutorial", tabName = "tutorial_tab"),
      menuItem("ANOVA vs LMER", tabName = "avl_tab"),
      menuItem("False Positives & Power", tabName = "fpp_tab"),
      menuItem("About", tabName = "about_tab"),
      actionButton("reset", "Reset"),
      tags$a(href="https://github.com/debruine/mixed-effects-tutorial/tree/master/mixed-effects", "Code for this app")
    )
  ),
  dashboardBody(
    useShinyjs(),
